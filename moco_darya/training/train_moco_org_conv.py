...
SECOND_SAVE_DIR = "/mnt/nas7/data/Personal/Darya/saved_models/moco_conv"
CHECKPOINT_SAVE_DIR = "/mnt/nas7/data/Personal/Darya/Checkpoints/moco_conv" 
PLOT_SAVE_DIR = "/home/darya/Histo_pipeline/Loss_curve_plot"
CSV_SAVE_PATH = "/home/darya/Histo_pipeline/MoCo_org_conv.csv"
...
def save_checkpoint(epoch, model, optimizer, scaler, base_lr, checkpoint_path, best=False):
    ...
    if best:
        best_path = os.path.join(CHECKPOINT_SAVE_DIR, "moco_best_model_conv.pth")
...
def load_existing_losses(plot_save_dir):
    ...
    loss_file = os.path.join(plot_save_dir, f"MOCO_org_loss_curve_conv.txt")
...
def train_moco(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Save checkpoint
        checkpoint_path = os.path.join(SECOND_SAVE_DIR, f"moco_org_conv_{epoch}.pth")
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = os.path.join(CHECKPOINT_SAVE_DIR, f"moco_checkpoint_epoch_{epoch+1}_conv.pth")
            save_checkpoint(epoch, model, optimizer, scaler, learning_rate, checkpoint_path)
    ...
    train_plot_path = os.path.join(PLOT_SAVE_DIR, f"MOCO_org_loss_curve_conv.png")
...
if __name__ == "__main__":
    ...
    moco_model = MoCoV2ConvNeXt(base_encoder='convnext_base', output_dim=128, queue_size=65536)  # Reduce from 65536 32768
