...
MODEL_SAVE_DIR = "/mnt/nas7/data/Personal/Darya/saved_models/superpixels_moco_org"
CHECKPOINT_SAVE_DIR = "/mnt/nas7/data/Personal/Darya/Checkpoints/superpixel_moco_org" 
PLOT_SAVE_DIR = "/home/darya/Histo_pipeline/Loss_curve_plot"
CSV_SAVE_PATH = "/home/darya/Histo_pipeline/Superpixel_moCo_org.csv"
...
def save_checkpoint(epoch, model, optimizer, scaler, base_lr, checkpoint_path, best=False):
    ...
    if best:
        best_path = os.path.join(CHECKPOINT_SAVE_DIR, "superpixel_moco_best_model.pth")
        torch.save(checkpoint, best_path)
        logger.info(f"New best model saved: {best_path}")

def get_latest_checkpoint(checkpoint_dir):
    checkpoints = sorted(glob.glob(os.path.join(checkpoint_dir, "superpixel_moco_org_*.pth")), key=os.path.getmtime, reverse=True)
    return checkpoints[0] if checkpoints else None  

def load_existing_losses(plot_save_dir):
    """Load previous loss values if they exist."""
    loss_file = os.path.join(plot_save_dir, f"superpixel_MOCO_loss_curve.txt")
...
def save_losses(losses, plot_save_dir):
    ...
    loss_file = os.path.join(plot_save_dir, "superpixel_MOCO_loss_curve.txt")
...
def train_moco(
    ...
    train_dataset = SuperpixelMoCoDataset(json_path, transform=train_transform)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=28, pin_memory=True, prefetch_factor=8)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (images_q, images_k) in enumerate(train_loader):
            ...
            logger.info(f"Epoch {epoch + 1}/{epochs}, Batch {batch_idx + 1}/{len(train_loader)}, Loss: {loss.item():.4f}, Average time per batch: {(time.perf_counter() - epoch_start_time)/(batch_idx+1):.6f}s")
        ...
        # Save checkpoint
        checkpoint_path = os.path.join(MODEL_SAVE_DIR, f"superpixel_moco_org_{epoch}.pth")
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = os.path.join(CHECKPOINT_SAVE_DIR, f"superpixel_moco_checkpoint_epoch_{epoch+1}.pth")
            save_checkpoint(epoch, model, optimizer, scaler, learning_rate, checkpoint_path)
    ...
    train_plot_path = os.path.join(PLOT_SAVE_DIR, f"Superpixel_MOCO_org_loss_curve.png")
